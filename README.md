...
host-agnostic C that an embedding environment integrates by providing a small
configuration header, and it presents the same station model as that driver --
...
- `src/` -- the driver:
  ...
  - `mm_halow_libc.c` -- weak newlib syscall stubs for `-nostdlib` hosts.
...
3. Link the prebuilt `morselib` and the transceiver firmware blob --
   ...
   newlib: if the host links `-nostdlib`, group libc/libm with it (the
   fragment does) and keep `mm_halow_libc.c`, whose weak stubs satisfy the
   syscall layer without touching the host heap.
