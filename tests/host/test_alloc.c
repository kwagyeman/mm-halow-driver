 ...
 * compile the real mm_halow_osal.c against the stubs in stub/, and call
 * mm_halow_heap_verify() after every operation to check the block list invariants.
 */
...
int mm_halow_test_atomic_depth = 0;
int mm_halow_test_atomic_max = 0;
uint32_t mm_halow_test_ticks_ms = 0;
int mm_halow_test_malloc_fail = 0;

#include "mm_halow_configport.h"

// Lets a test force allocation failure and prove the driver degrades instead
// of crashing -- the on-device failure mode with no debugger attached.
uint8_t *mm_halow_port_heap_alloc(size_t n) {
    if (mm_halow_test_malloc_fail) {
        return NULL;
    }
    return calloc(1, n);
}
void mm_halow_port_heap_free(uint8_t *p) {
    free(p);
}

// The scheduler is ARM/PendSV and cannot build on the host. mm_halow_osal.c
...
typedef struct _mm_halow_task_t mm_halow_task_t;
typedef bool (*mm_halow_cond_fn_t)(void *arg);
mm_halow_task_t *mm_halow_sched_task_create(void (*entry)(void *), void *arg, size_t sw, const char *n) {
    (void)entry;
    (void)arg;
    (void)sw;
    (void)n;
...
void mm_halow_sched_task_delete(mm_halow_task_t *t) {
    (void)t;
}
...
bool mm_halow_sched_wait(mm_halow_cond_fn_t c, void *a, uint32_t t) {
    (void)c;
    (void)a;
    (void)t;
    return true;
...
static void mm_halow_heap_verify(const char *where) {
    checks++;
    size_t total = 0;
    int blocks = 0;
    const uint8_t *base = (const uint8_t *)mm_halow_heap_mem;
    const uint8_t *end = base + MM_HALOW_HEAP_SIZE;
    ...
    for (mm_halow_block_t *b = mm_halow_heap_head; b != NULL; b = b->next) {
        ...
        if ((const uint8_t *)b + MM_HALOW_BLOCK_HDR + b->size > end) {
            failures++;
        ...
        if (b->next != NULL) {
            const uint8_t *expect = (const uint8_t *)b + MM_HALOW_BLOCK_HDR + b->size;
        ...
        if (((uintptr_t)MM_HALOW_BLOCK_DATA(b) % MM_HALOW_BLOCK_ALIGN) != 0) {
            failures++;
            printf("  FAIL %s: payload %p not %d-aligned\n", where,
                MM_HALOW_BLOCK_DATA(b), MM_HALOW_BLOCK_ALIGN);
            return;
        }
        total += MM_HALOW_BLOCK_HDR + b->size;
    }
    // Every byte accounted for: no leaked space between blocks.
    if (total != MM_HALOW_HEAP_SIZE) {
        failures++;
        printf("  FAIL %s: blocks account for %zu of %d bytes\n", where,
            total, MM_HALOW_HEAP_SIZE);
    }
}

static void setup(void) {
    mm_halow_test_malloc_fail = 0;
    mm_halow_test_atomic_depth = 0;
    CHECK(mm_halow_osal_init(), "init should succeed");
}
static void teardown(void) {
    mm_halow_osal_deinit();
    CHECK(mm_halow_test_atomic_depth == 0, "atomic sections unbalanced: depth %d",
...
static void test_init_deinit(void) {
    setup();
    CHECK(mm_halow_heap_mem != NULL, "heap pointer should be set");
    CHECK(mm_halow_heap_head != NULL, "head should be set");
    mm_halow_heap_verify("after init");
    teardown();
    CHECK(mm_halow_heap_mem == NULL, "heap pointer cleared on deinit");
    // The dangling-head hazard: a stale mm_halow_heap_head after deinit would make
    // the next malloc walk freed memory. active(False)/active(True) does this.
    CHECK(mm_halow_heap_head == NULL, "head must be cleared on deinit, else the "
        "next active(True) walks freed memory");
}

static void test_init_is_idempotent(void) {
    setup();
    void *first = mm_halow_heap_mem;
    CHECK(mm_halow_osal_init(), "second init should succeed");
    CHECK(mm_halow_heap_mem == first, "second init must not re-allocate");
    teardown();
}

static void test_init_handles_oom(void) {
    mm_halow_osal_deinit();
    mm_halow_test_malloc_fail = 1;
    CHECK(!mm_halow_osal_init(), "init must report failure, not crash, when the GC "
        "heap cannot give up 96 KB");
    CHECK(mm_halow_heap_mem == NULL, "no heap on failed init");
    CHECK(mm_halow_osal_malloc(64) == NULL, "malloc must return NULL with no pool");
    mm_halow_osal_free(NULL);
    mm_halow_test_malloc_fail = 0;
}

static void test_basic_alloc_free(void) {
    setup();
    void *a = mm_halow_osal_malloc(100);
    CHECK(a != NULL, "100-byte alloc should succeed");
    mm_halow_heap_verify("after alloc");
    memset(a, 0xAA, 100);
    mm_halow_osal_free(a);
    mm_halow_heap_verify("after free");
    teardown();
}

static void test_zero_size(void) {
    setup();
    CHECK(mm_halow_osal_malloc(0) == NULL, "malloc(0) returns NULL");
    mm_halow_heap_verify("after malloc(0)");
    teardown();
}
...
static void test_alignment(void) {
    ...
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        p[i] = mm_halow_osal_malloc(sizes[i]);
        CHECK(p[i] != NULL, "alloc %zu failed", sizes[i]);
        CHECK(((uintptr_t)p[i] % MM_HALOW_BLOCK_ALIGN) == 0,
            "alloc %zu returned %p, not %d-aligned", sizes[i], p[i], MM_HALOW_BLOCK_ALIGN);
    }
    mm_halow_heap_verify("after ragged allocs");
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        mm_halow_osal_free(p[i]);
    }
    mm_halow_heap_verify("after ragged frees");
    teardown();
}
...
static void test_no_overlap_and_writes_are_isolated(void) {
    ...
    mm_halow_heap_verify("after N frees");
    teardown();
}

static void test_coalesce_restores_full_heap(void) {
    setup();
    size_t biggest_before = mm_halow_heap_head->size;
    enum { N = 32 };
    void *p[N];
    for (int i = 0; i < N; i++) {
        p[i] = mm_halow_osal_malloc(512);
        CHECK(p[i] != NULL, "alloc %d failed", i);
    }
    ...
    mm_halow_heap_verify("after even frees");
    for (int i = 1; i < N; i += 2) {
        mm_halow_osal_free(p[i]);
    }
    mm_halow_heap_verify("after odd frees");
    CHECK(mm_halow_heap_head->next == NULL, "heap should be one block again, got a list");
    CHECK(mm_halow_heap_head->size == biggest_before,
...
static void test_exhaustion_returns_null(void) {
    ...
    int n = 0;
    while (mm_halow_osal_malloc(4096) != NULL) {
        n++;
        if (n > 1000) {
            break;
        }
    ...
    CHECK(mm_halow_osal_malloc(4096) == NULL, "exhausted heap must return NULL");
    CHECK(mm_halow_osal_malloc(1) == NULL || 1, "small alloc may still fit; not fatal");
    mm_halow_heap_verify("at exhaustion");
    teardown();
}

static void test_too_big_returns_null(void) {
    setup();
    CHECK(mm_halow_osal_malloc(MM_HALOW_HEAP_SIZE * 2) == NULL,
        "an allocation larger than the pool must return NULL");
    mm_halow_heap_verify("after oversize request");
    teardown();
}

// Independent of calloc: any caller passing a size near SIZE_MAX corrupts the
// heap, because MM_HALOW_BLOCK_ROUND wraps. morselib sizes buffers from wire
...
static void test_malloc_size_overflow(void) {
    ...
    void *p = mm_halow_osal_malloc(rounds_to_zero);
    CHECK(p == NULL, "malloc(SIZE_MAX-3) must return NULL; rounding wrapped to 0 "
        "and handed back %p", p);
    CHECK(mm_halow_osal_malloc(SIZE_MAX) == NULL, "malloc(SIZE_MAX) must return NULL");
    mm_halow_heap_verify("after size-overflow requests");
    teardown();
}
...
static void test_calloc_zeroes(void) {
    ...
    mm_halow_osal_free(p);
    mm_halow_heap_verify("after calloc/free");
    teardown();
}
...
static void test_calloc_overflow(void) {
    ...
    mm_halow_heap_verify("after overflow attempts");
    teardown();
}
...
static void test_realloc(void) {
    ...
    void *p = mm_halow_osal_malloc(64);
    memset(p, 0x5A, 64);
    ...
    mm_halow_heap_verify("after realloc");
    teardown();
}
...
static void test_stress(void) {
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        if (p[slot] == NULL) {
            ...
            p[slot] = mm_halow_osal_malloc(want);
            if (p[slot] != NULL) {
                sz[slot] = want;
            ...
            mm_halow_osal_free(p[slot]);
            p[slot] = NULL;
            frees++;
        }
        if ((i % 500) == 0) {
            mm_halow_heap_verify("during stress");
        }
    }
    checks++;
    ...
    mm_halow_heap_verify("after stress");
    CHECK(mm_halow_heap_head->next == NULL,
