...
static inline uint32_t mm_halow_test_ticks(void) {
    return mm_halow_test_ticks_ms;
}
#define mm_halow_ticks_ms mm_halow_test_ticks

// The allocator tests exercise no pins; satisfy the contract with no-ops.
#define mm_halow_hal_pin_write(pin, v) ((void)(pin), (void)(v))
#define mm_halow_hal_pin_read(pin) ((void)(pin), 0)
#define mm_halow_hal_pin_output(pin) ((void)(pin))
#define mm_halow_hal_pin_input(pin) ((void)(pin))

// The host harness never runs from interrupt context.
#define mm_halow_in_irq() (false)
